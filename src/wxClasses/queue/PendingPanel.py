...
class PendingPanel(wx.Panel):
    ...
    def _create_widgets(self):
        ...
        asset_column = ColumnDefn("Asset", "left", 60, "asset", isSpaceFilling=True)
        process_column = ColumnDefn("Process", "right", 100, "process")
        status_column = ColumnDefn("Status", "right", 80, "status")

        columns = [asset_column, process_column, status_column]

        self.olv = ObjectListView(parent=self, style=wx.LC_REPORT | wx.SUNKEN_BORDER,
                                  useAlternateBackColors=True, sortable=False)

        assets = self.data.db.queue.pending
        logging.debug(f'pending panel assets: {assets}')

        self.olv.SetEmptyListMsg('No pending items in Queue')
